...
ConfidenceIntervalsTS = function(ts,year){
  ...
  for(tau in minseglen:(n-minseglen)){
    ...
    test.stat[tau,1]=first[1]+secondseg(ts[(tau+1):n],start=tau,previousbeta=first[2])[1]
  }
  
  # above is fast despite being a double loop :-)
  
  # changepoint times
  year[apply(test.stat,MARGIN=2,FUN=which.min)]
  ...
  apply(test.stat,MARGIN=2,FUN=function(x){
    ...
    CI = year[which(x<min(x,na.rm=T)+CS.thresh)]
    return(CI)
  })
}
